<template>
  <section class="section">
    <div class="container no-print">
      <h1 class="title">ຮັບຊຳລະ ຄ່າທຳນຽມທາງ</h1>
      <LicensePlateInputForm
        v-if="showInputLicense"
        @on-license-plate-inputed="onLincensePlateInputed"
      />
      <div v-if="vehicleInfo">
        <LicencePlate
          :type_id="vehicleInfo.purpose_no"
          :license_no="vehicleInfo.license_no"
          :province_id="vehicleInfo.province_code"
          @click.native="clearPaymentForm"
        />
      </div>
      <div class="columns" v-if="vehicleInfo">
        <div class="column">
          <div class="box">
              ...
              <div class="column">
                <b-field label="ຊຳລະດ້ວຍ" expanded>
                  <b-select
                    placeholder="ເລືອກ"
                    expanded
                    v-model="inputPaymentType"
                    name="inputPaymentType"
                    v-validate="'required'"
                  >
                    <option value="cash">ເງິນສົດ</option>
                    <option value="acc">ຕັດບັນຊີ</option>
                  </b-select>
                </b-field>
              </div>
              <div class="column">
                <b-field label="ເລກບັນຊີ" expanded>
                  <b-field>
                    <b-input placeholder="ເລກບັນຊີ" expanded :disabled="inputPaymentType!=='acc'"></b-input>
                    <p class="control">
                      <button
                        class="button is-primary"
                        :disabled="inputPaymentType!=='acc' && inputAccountNo.length < 5"
                        @click="onCheckSignatureClicked"
                      >ກວດລາຍເຊັນ</button>
                    </p>
                  </b-field>
                </b-field>
            ...
            <b-field label="ປີທີ່ຈະຊຳລະ">
              <table class="table is-fullwidth" v-if="vehicleInfo.payabletax.length > 0">
                <thead>
                  <tr>
                    <th>ປີ</th>
                    <th>ຄ່າທຳນຽມ</th>
                ...
                <tbody>
                  <tr
                    v-for="(row, index) in vehicleInfo.payabletax"
                    :key="row.tax.tax_year"
                    :class="{'is-selected': isSelected(row)}"
                    @click="onTaxClicked(index)"
                  >
                    <td>{{row.tax.tax_year}}</td>
                    <td>{{row.tax.amt | formatNumberLocal}}</td>
                    <td>{{row.fine.fineamount | formatNumberLocal}}</td>
                    <td>{{row.fine.finenote}}</td>
                    <td width="150">{{isSelected(row) ?'ເລືອກ': '-'}}</td>
                  </tr>
                </tbody>
              </table>
              <div class="box" v-else>ບໍ່ມີຄ່າທຳນຽມ ທີ່ຕ້ອງຊຳລະ</div>
            </b-field>
            <b-field label="ປີທີ່ເລືອກ">
              <b-input
                :value="_.map(selectedTaxItems,(o) => {return o.tax.tax_year}).join(',')"
                expanded
                disabled
              ></b-input>
            </b-field>

            <b-field label="ລວມເງິນທີ່ຕ້ອງຈ່າຍ">
              <b-input
                :value="_.map(selectedTaxItems,(o) => {return o.tax.amt + o.fine.fineamount}).reduce((sum, n) => sum + n, 0) | formatNumberLocal"
                expanded
                disabled
              ></b-input>
            </b-field>
              ...
              <div class="columns">
                ...
                <div class="column">
                  <button
                    ...
                    :disabled="!stickers || allPrinted"
                    @click="printSticker"
                  >ພິມສະຕິກເກີ້</button>
                </div>
                <div class="column">
                  <button class="button is-fullwidth is-default" @click="clearPaymentForm">ເລີ່ມໃໝ່</button>
                </div>
              </div>
            </b-field>
          </div>
...
export default {
  ...
  methods: {
    ...
    async printSticker() {
      ...
      for (let i = 0; i < this.stickers.length; i++) {
        ...
        const stickerinfo = {
          ...
          taxamount:
            this.$options.filters.formatNumberLocal(print_amt) + " ກີບ",
          sn: seq_no,
          chassisno: chassis_no,
          printcount: print_count,
          purposeno: purpose_no,
          platecolor:
            this.$options.filters.purposeDesc(purpose_no) + " - " + category_id
        };
    ...
    clearPaymentForm() {
      ...
      (this.form = null), (this.stickers = null);
    },
    onTaxClicked(index) {
      //alert(index)
      const selected = this.vehicleInfo.payabletax[index];
      const exist = this._.find(this.selectedTaxItems, o => {
        return o.tax.tax_year === selected.tax.tax_year;
      });
      if (exist !== undefined && index === this.selectedTaxItems.length - 1) {
        this.selectedTaxItems.splice(index, 1);
      } else if (index === this.selectedTaxItems.length) {
        this.selectedTaxItems.push(selected);
      }
    },
    isSelected(search) {
      const exist = this._.find(this.selectedTaxItems, o => {
        return o.tax.tax_year === search.tax.tax_year;
      });
    ...
    async onSaveClicked() {
      ...
      if (
        this.inputPaymentType === "acc" &&
        this.user.agentid === "BCEL" &&
        this.inputAccountNo.length === 0
      ) {
        this.errors.add({
      ...
      let req = {
        ...
        payment_requests: this.selectedTaxItems.map(item => {
          return {
            idx: this.vehicleInfo.idx,
            tax_year: item.tax.tax_year,
            tax_amt: item.tax.amt,
            fine_amt: item.fine.fineamount,
            fine_note: item.fine.fineanote,
