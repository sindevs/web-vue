<template>
    <section class="section">
        <div class="container">
            <h1 class="title">ດັດແກ້ ການຊຳລະ (ກໍລະນີ ຊຳລະຜິດຄັນ)</h1>

            <div class="section">
                <div class="box">
                    <h1 class="title is-5">ທະບຽນທີ່ຊຳລະຜິດ</h1>
                    <button class="button is-primary" @click="licenseDestination =0; showInputLicenseModal = true">
                        ເລືອກປ້າຍ
                    </button>
                    <div class="columns" v-if="oldLicense">
                        <div class="column">
                            <div>
                        ...
                        <div class="column">
                            <b-table
                                    :data="oldLicense.paymenthistory"
                                    :checked-rows.sync="oldCheckedRows"
                                    checkable>
                                <template slot-scope="props">
                                    <b-table-column field="seq_no" label="SN" width="40" numeric>
                                        {{ props.row.seq_no }}
                                    </b-table-column>
                                    <b-table-column field="payment_type" label="ວັນທີ">
                                        {{ props.row.payment_date }}
                                    </b-table-column>
                                    <b-table-column field="tax_year" label="ສົກປີ">
                                        {{ props.row.tax_year }}
                                    </b-table-column>
                                    <b-table-column field="tax_amt" label="ມູນຄ່າ">
                                        {{ props.row.tax_amt }}
                                    </b-table-column>
                                    <b-table-column field="fine_amt" label="ຄ່າປັບ">
                                        {{ props.row.fine_amt }}
                                    </b-table-column>
                                </template>

                                <template slot="bottom-left">
                                    <b>Total checked</b>: {{ oldCheckedRows.length }}
                                </template>
                            </b-table>
                        </div>
                    </div>
            ...
            <div class="section">
                <div class="box">
                    <h1 class="title is-5">ທະບຽນ ທີ່ຕ້ອງການຊຳລະ</h1>
...
<script>
	...
	export default {
		...
		components: {LicensePlateInputForm, CompactVehicleInfo},
		data() {
			return {
			...
			onLincensePlateInputed(license_no, province_code, purpose_no) {
				...
				.then(res => {
					...
					if (this.vehicleInfos.length == 1) {
						...
						this.showInputLicenseModal = true
					}
				})
			},
